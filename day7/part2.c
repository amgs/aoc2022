...
int smallest_size_over(Folder folder, long target_size);

int main() {
    Folder root = read_info("day7/input.txt");
    int current_free_space = CAPACITY - folder_size(root);
    int space_to_free = REQUIRED_SPACE - current_free_space;
    int largest_size = smallest_size_over(root, space_to_free);
